        ...
        <div className="md:hidden bg-canvas px-6 py-8 space-y-4 border-b border-veil">
          {navLinks.map((link) => (
            <a
              ...
              className="block text-xs font-sans uppercase tracking-[0.15em] text-clay hover:text-bark py-1 transition-colors"
            >
              {link.label}
            </a>
          ))}
          <a
            href={COMPANY_INFO.phoneHref}
            className="flex items-center gap-2 pt-4 text-xs font-sans tracking-wide text-clay border-t border-veil mt-4"
          >
